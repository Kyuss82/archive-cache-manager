...
{
    class CreateTadMenuItem : IGameMenuItemPlugin
    {
        public bool SupportsMultipleGames => false;
        public string Caption => "Create TAD Package...";
        public Image IconImage => Resources.icon16x16;
        public bool ShowInLaunchBox => true;
        public bool ShowInBigBox => false;

        public bool GetIsValidForGame(IGame selectedGame) =>
            selectedGame != null && Config.MatchesTadPlatform(selectedGame.Platform);
        ...
        public void OnSelected(IGame selectedGame)
        {
            ...
            try
            {
                ...
                var results = TadStaging.BuildFromGameArchive(archivePath, outputBaseDir, baseName);

                if (Config.TadAddToLibrary)
                {
                    AddTadsToLibrary(selectedGame, results);
                }
            ...
            catch (Exception ex)
            {
                ...
                UserInterface.ErrorDialog("Unexpected error creating TAD. See log for details.");
            }
        }

        private static void AddTadsToLibrary(IGame sourceGame, List<TadBuildResult> results)
        {
            ...
            foreach (var r in successes)
            {
                try
                {
                    ...
                    newGame.Source = "Archive Cache Manager (TAD)";
                    added++;
                }
                catch (Exception ex)
                {
                    Logger.Log(string.Format("Failed to add TAD to library ({0}): {1}", r.OutputPath, ex.Message), Logger.LogLevel.Exception);
                }
            }
            ...
            {
                Logger.Log(string.Format("Failed to save library after adding TADs: {0}", ex.Message), Logger.LogLevel.Exception);
            }
        }

        private static string ResolveOutputBaseDir(string archivePath)
        {
            string configured = Config.TadOutputPath;
        ...
        private static void ReportResults(List<TadBuildResult> results)
        {
            ...
            {
                MessageBox.Show(
                    string.Format("TAD creation completed.\r\n\r\n{0}", message),
                    "Create TAD Package",
            ...
            else
            {
                UserInterface.ErrorDialog(
                    string.Format("TAD creation finished with {0} success(es) and {1} failure(s).\r\n\r\n{2}",

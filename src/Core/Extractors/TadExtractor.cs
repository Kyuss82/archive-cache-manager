...
{
    /// <summary>
    /// On-launch wrapper that assembles DSi .tad files inside the plugin cache from a CDN-dump archive.
    /// Underlying pipeline is TadStaging. When the archive carries multiple TMDs, every successful TAD
    /// is kept in cache and the highest-version one is flagged as SelectedFile.
    /// </summary>
    public class TadExtractor : Extractor
    {
        ...
        public override bool Extract(string archivePath, string cachePath, string[] includeList = null, string[] excludeList = null)
        {
            ...
            var results = TadStaging.BuildFromGameArchive(archivePath, cachePath, baseName);
            ...
            if (successful.Count == 0)
            {
                ...
                Logger.Log(string.Format("TadExtractor: build failed: {0}", detail));
                return false;
            }

            var primary = successful.OrderByDescending(r => r.TmdVersion).First();
            Logger.Log(string.Format("TadExtractor: primary TAD = {0} (TMD v{1})", primary.OutputPath, primary.TmdVersion));

    ...
    public class CacheManager
    {
        ...
        static long GetArchiveCachePlaytime(string archiveCachePath)
        {
            ...
            if (File.Exists(playTimePath))
            {
                try
                {
                    using (StreamReader reader = new StreamReader(playTimePath))
                    {
        ...
        public static void UpdateArchiveCachePlaytime()
        {
            ...
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(lastPlayedPath, false))
                {
        ...
        public static void BatchCacheArchive(string[] args)
        {
            ...
            if (args.Length > 1 && long.TryParse(args[1], out long size))
            {
                LaunchInfo.SetSize(size);
            }
            ExtractArchive(args);
        }
    }

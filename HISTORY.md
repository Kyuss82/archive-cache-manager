...
## v2.80 (Kyuss82 fork)
* **Wii joins the Local Mirror Indexer + library-purge pipeline.** New `LocalPkgPlatform.Wii` value plumbed through every switch site (`LocalPkgIndexer.BuildIndex` / `CountFiles` / `ResolveManifestPath` / `FoldersForPlatform`, plus `LocalPkgIndexerWindow.PersistFolders`). Scope is **`.wad` files only** — WiiWare, Virtual Console, system channels, hidden channels, DLC. Disc-based games (`.iso`/`.wbfs`/`.rvz`) are intentionally out of scope: those are the base library entries the user wants to keep, and the purge feature would otherwise have to skip-list them anyway.
* **`ScanWii` in `LocalPkgScanner.cs`.** Walks `*.wad` files recursively. For each, parses the `.wad` header (big-endian, 0x40-aligned sections per wiibrew.org/wiki/WAD_files), computes the TMD offset as `alignUp(0x40 + cert_size, 0x40) + alignUp(ticket_size, 0x40)`, reads the title_id at TMD offset `0x18C` (8 bytes BE, same TMD layout as Wii U / 3DS — that's shared Nintendo crypto plumbing) and the title_version at `0x1DC`. Header parsing is defensive: accepts any 4-byte type tag (not just `"Is\0\0"`), validates `header_size == 0x20`, rejects truncated TMDs, and emits a `[skip] … WAD header unparseable` line rather than throwing on malformed input. New private `AlignUp` helper.
* **`AddWiiEntry` categorization** uses the `title_id_high` upper 32 bits (per wiibrew.org/wiki/Titles):
  * `0x00000001` → `SystemTitle` (boot2, IOS, MIOS, System Menu — the bootable system layer)
  * `0x00010000` → `Skip` (disc save data — not a redistributable title)
  * `0x00010001` → `Skip` (downloadable WiiWare / VC base — already in the LB library if the user imported it as a game)
  * `0x00010002` → `SystemTitle` (system channels: News, Forecast, Mii, Photo, etc.)
  * `0x00010004` → `Other` (game-with-channel installers — Mario Kart Wii etc. install their own channel as a side-effect of running)
  * `0x00010005` → `DLC` (downloadable game content)
  * `0x00010008` → `SystemTitle` (hidden channels: EULA, Region Select, etc.)
  * default → `Other` (surface for review rather than swallow)
* **Bucket grouping rule for Wii** diverges slightly from Wii U / 3DS: `DLC` and game-with-channel `Other` entries attach to a parent base game whose TID is `00010001` + low (same low 32 bits as the WiiWare/VC base they extend). Everything else (system content, IOS, channels, unknown) groups under **its own** 16-hex title_id — there is no game association to inherit, so the entry forms its own bucket. This keeps `manifest.Titles` lookups consistent (one entry per logical title) while still letting the purge UI surface system content as standalone rows.
* **New right-click menu item `Index Local Wii ROM Folders…`** (`Plugin/WiiLocalRomIndexerMenuItem.cs`, parallel to the existing `WiiuLocalRomIndexerMenuItem` / `Ctr3dsLocalRomIndexerMenuItem`). Visibility gate uses `Config.MatchesWadPlatform()` (the same predicate the right-click *Create Wii Package…* and on-launch `WadExtractor` use). Opens `LocalPkgIndexerWindow` with the new **Wii** tab pre-selected.
* **`LocalPkgIndexerWindow` Wii tab**: shows up automatically because the platforms array gained `LocalPkgPlatform.Wii`. The tab inherits all the generic UI behaviour (folder list, drag-drop from Explorer, real progress bar, log filters, manifest tree) — no per-platform UI specialisation was needed beyond the new `PersistFolders` switch case.
* **Config plumbing**: new `Config.WiiLocalRomFolders` (CSV string, parallel to the existing `WiiuLocalRomFolders` / `Ctr3dsLocalRomFolders`). Default empty. Wired through the load section, save section, and reset section of `Config.cs`. The corresponding `[Archive Cache Manager]` INI key is `WiiLocalRomFolders`.
* **Manifest path**: persisted at `<plugin>/WiiLocalCache/local_rom_index.json`, parallel to `WiiuLocalCache/` and `Ctr3dsLocalCache/`. The directory is auto-created on first save by `LocalPkgIndexer.Save`.
* **Library purge picks Wii up automatically.** `LibraryPurgeCandidates.EnumerateAll()` iterates every value of `LocalPkgPlatform`, so the new manifest is read on the next purge-window open with zero further changes. The purge menu's visibility gate (`PurgeUpdateDlcMenuItem.GetIsValidForGame`) now also matches `Config.MatchesWadPlatform`, so the right-click entry appears on Wii games too.
* **Caveats:**
  * Coverage is downloadable `.wad` only. Disc games stay in the library untouched — the purge cannot accidentally remove them because they're not in the manifest.
  * NUS dumps (`tmd.NN` + `cetk` + `NNNNNNNN.app` loose-file form) are not currently scanned for Wii. If anyone reports needing this, a Wii NUS scanner parallel to `IsWiiuTmdFile` is a small follow-up; until then, users should `Sharpii-NetCore` their NUS dumps into `.wad` first.
  * No ticket / title-key reading is required by the indexer (we only need title_id from the TMD), so cert/key requirements that apply to the `.wad` builder don't apply here — indexing works on any `.wad` regardless of whether the user has `cert.sys` / title-key DB configured.
* **Files touched:** `Core/Config.cs` (+5 spots for `WiiLocalRomFolders`), `Core/Packagers/LocalPkgManifest.cs` (enum +Wii), `Core/Packagers/LocalPkgIndexer.cs` (BuildIndex/CountFiles/ResolveManifestPath/FoldersForPlatform), `Core/Packagers/LocalPkgScanner.cs` (`ScanWii` + `ReadWadTitleId` + `AddWiiEntry` + `AlignUp` helper), `Plugin/WiiLocalRomIndexerMenuItem.cs` (new), `Plugin/LocalPkgIndexerWindow.cs` (platforms array + PersistFolders switch), `Plugin/PurgeUpdateDlcMenuItem.cs` (+MatchesWadPlatform in the visibility gate).
